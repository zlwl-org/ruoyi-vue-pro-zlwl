    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['shop:brand:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['shop:brand:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['shop:brand:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['shop:brand:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="品牌图片" prop="picUrl">
          <el-input v-model="form.picUrl" placeholder="请输入品牌图片" />
        </el-form-item>
...
<script>
import { createBrand, updateBrand, deleteBrand, getBrand, getBrandPage, exportBrandExcel } from "@/api/shop/brand";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除商品品牌编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有商品品牌数据项?').then(() => {
          this.exportLoading = true;
          return exportBrandExcel(params);
        }).then(response => {
          this.$download.excel(response, '商品品牌.xls');
