  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="产品编号" prop="productId">
        <el-input v-model="queryParams.productId" placeholder="请输入产品编号" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="品牌编号" prop="brandId">
        <el-input v-model="queryParams.brandId" placeholder="请输入品牌编号" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['shop:branch-goods:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['shop:branch-goods:export']">导出</el-button>
      </el-col>
...
<script>
import { createBranchGoods, updateBranchGoods, deleteBranchGoods, getBranchGoods, getBranchGoodsPage, exportBranchGoodsExcel } from "@/api/shop/branchGoods";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getBranchGoods(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        if (this.form.id != null) {
          updateBranchGoods(this.form).then(response => {
        ...
        // 添加的提交
        createBranchGoods(this.form).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除门店商品编号为"' + id + '"的数据项?').then(function() {
          return deleteBranchGoods(id);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有门店商品数据项?').then(() => {
          this.exportLoading = true;
          return exportBranchGoodsExcel(params);
        }).then(response => {
          this.$download.excel(response, '门店商品.xls');
