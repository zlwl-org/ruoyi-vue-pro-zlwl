  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="电话" prop="tel">
        <el-input v-model="queryParams.tel" placeholder="请输入电话" clearable @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['shop:branch:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['shop:branch:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['shop:branch:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['shop:branch:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createBranch, updateBranch, deleteBranch, getBranch, getBranchPage, exportBranchExcel } from "@/api/shop/branch";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getBranchPage(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createBranch(this.form).then(response => {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有门店数据项?').then(() => {
          ...
          return exportBranchExcel(params);
        }).then(response => {
